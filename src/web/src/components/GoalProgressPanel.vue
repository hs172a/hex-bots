  ...
  <div class="bg-[#0d1117] border border-space-border rounded-lg p-3 space-y-2 text-xs">
    ...
    <div class="flex items-center justify-between gap-2">
      <div class="min-w-0">
        ...
        <div class="text-[11px] text-space-text-dim capitalize">{{ goal.type }} build goal</div>
      </div>
      <span class="px-1.5 py-0.5 rounded text-[11px] font-medium shrink-0" :class="statusClass(goal.status)">
    ...
    <div class="flex items-center gap-2">
      ...
      <span class="text-[11px] text-space-text-dim shrink-0">{{ readyCount }}/{{ goal.materials.length }} items</span>
    </div>
    ...
    <div class="space-y-0.5 max-h-44 overflow-auto scrollbar-dark">
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          ...
          <!-- Source info -->
          <div v-if="mat.acquisition === 'buy' && mat.buy_source" class="text-[11px] text-space-text-dim mt-0.5 truncate">
          ...
          <div v-else-if="mat.acquisition === 'mine'" class="text-[11px] text-space-text-dim mt-0.5">
            ⛏️ Mine{{ mat.mine_source?.system_name ? ' in ' + mat.mine_source.system_name : ' at asteroid belt' }}
          </div>
          <div v-else-if="mat.acquisition === 'unknown' && mat.quantity_have < mat.quantity_needed" class="text-[11px] text-orange-400/80 mt-0.5">
    ...
    <div class="flex items-center gap-1.5 pt-1 border-t border-[#21262d]">
      <span class="text-[11px] text-space-text-dim shrink-0">Bot:</span>
      <select
        :value="goal.assigned_bot"
        @change="emit('assign', ($event.target as HTMLSelectElement).value)"
        class="flex-1 bg-space-bg border border-space-border rounded px-1.5 py-0.5 text-[11px] text-space-text focus:border-space-accent outline-none min-w-0"
      >
      ...
      <button
        ...
        @click="emit('resume')"
        class="btn text-[11px] px-2 py-0.5 whitespace-nowrap"
      >▶ Resume</button>
      <button
      ...
      <button
        ...
        @click="emit('build')"
        class="btn btn-primary text-[11px] px-2 py-0.5 whitespace-nowrap"
      >🔨 Build!</button>

      <button
        @click="emit('delete')"
        title="Remove goal"
        class="btn text-[11px] px-2 py-0.5 text-red-400 border-red-900/40 hover:bg-red-900/20 whitespace-nowrap"
      >🗑</button>
    </div>

    <div v-if="hasUnknown" class="text-[11px] text-space-text-dim/70 italic">
